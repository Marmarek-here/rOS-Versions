# rOS_builds
rOS stable branch. Used for obtaining stable releases and source code.
...
BACK UP  YOUR DATA INTO A CLOUD (or safe place you are 100% sure you won't wipe).
1. Unlock the bootloader by tapping 7 times on "Build Number", go to Developer Options and there press "OEM Unlock", log out from your Google account to prevent FRP (Factory Reset Protection) and boot into Download Mode, Fastboot or whatever using a specific buttons combination.
2. Get a TWRP for you model. Flashing TWRP for another device or even the same device with a mismatched model code (like mistaking SM-N960F with another Note 9 version build) can or may result in a bootloop, a brick etc.
3. Get rOS file for your device. Flashing rOS for another device or even the same device with a mismatched model code (like mistaking SM-N960F with another Note 9 version build) can or may result in a bootloop, a brick etc.
4. Flash TWRP using Fastboot, Odin or any other recovery your phone has.
5. Boot into TWRP by holding different button combinations (they are different for some devices, like for Note 9 it's Vol- and Power buttons).
6. Flash
...
7. (If you want Gservices and Gapps) Find a proper Gapps for your Android version (example if you have rOS 1.0 then Android 17 etc.), architecture and the amount of Gservices and Gapps you want. I personally recommend MindTheGapps, but you don't need to install only it, you can use whatever Gapps you want!
